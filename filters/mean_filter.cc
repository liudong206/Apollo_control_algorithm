...
MF::MeanFilter(const uint8 window_size) {
    ...
    if(!(window_size_ > 0)){
        std::cout << "window_size_ <= 0" <<std::endl;
    }
    if(!(window_size_ <= kMaxWindowSize)){
        std::cout << "window_size_ > kMaxWindowSize" <<std::endl;
    }
    initialized_ = true;
}
...
double MF::Update(const double measurement) {
    if(initialized_ == false){
        std::cout << "init failed1" <<std::endl;
    }
    if(!(values_.size() <= window_size_)){
        std::cout << "value_.size() > window_size_" << std::endl;
    }
    if(!(min_candidates_.size() <= window_size_)){
        std::cout << "min_candidates_.size() > window_size_" << std::endl;
    }
    if(!(max_candidates_.size() <= window_size_)){
        std::cout << "max_candidates_.size() > window_size_" <<std::endl;
    }
  ++time_;
